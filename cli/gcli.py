...
def recommendations_content_distance(df,
    ...
    score_series = pd.Series(cosine_sim[0]).sample(frac=1).sort_values(ascending = False)
    score_series = score_series.where(lambda sim: sim != 0).dropna()
...
if __name__ == '__main__':
    ...
    # Argument parser configuation
    parser = argparse.ArgumentParser(description=f'Try out these categories: {category_choices}')
    parser.add_argument('--location',
        nargs=1,
        required=True,
        metavar='<city>, <state_code>',
        help='Enter a "City, StateCode" combination with proper capitalization: e.g. "San Jose, CA". Longitude, latitude coordinates are also accepted: e.g. "-76.23, 123"')
    parser.add_argument('--distance',
        nargs=1,
        type=int,
        metavar='<integer>',
        default=20,
        help='Get recommendations within this distance from starting location. Default to 20km')
    ...
    elif all(map(lambda x: not is_number(x), location)):
